...
public export
lexTok16 : List Char -> Integer -> SnocList Token -> Either LexErr (SnocList Token)

public export
lexTok17 : List Char -> SnocList String -> SnocList Token -> Either LexErr (SnocList Token)

public export
lexTok18 : List Char -> SnocList String -> SnocList Token -> Either LexErr (SnocList Token)

public export
lexTok19 : List Char -> SnocList Char -> SnocList String -> SnocList Token -> Either LexErr (SnocList Token)
...
lexTok4 str@(c::cs) x1 x0 =
  case c of
    ...
    _  => case validStrChar c of
      ...
      _   => lexTok13 str x1 x0
lexTok4 [] x1 x0 = lexTok13 Nil x1 x0
...
lexTok10 str@(c::cs) x1 x0 =
  case isIdentTrailing c of
    ...
    _    => lexTok17 str ((:<) Lin (pack ((<>>) x1 Nil))) x0
lexTok10 [] x1 x0 = lexTok17 Nil ((:<) Lin (pack ((<>>) x1 Nil))) x0
...
lexTok15 str@(c::cs) x1 x0 =
  case (/=) c (fromChar '\n') of
    True => lexTok15 cs ((:<) x1 c) x0
    _    => lexTok2 str ((:<) x0 (comment x1))
lexTok15 [] x1 x0 = lexTok2 Nil ((:<) x0 (comment x1))

lexTok16 str@(c::cs) x1 x0 =
  case (&&) ((<=) c '9') ((<=) '0' c) of
    True => lexTok16 cs ((+) ((*) x1 10) (toDigit c)) x0
    _    => lexTok2 str ((:<) x0 (IntegerLit (negate x1)))
lexTok16 [] x1 x0 = lexTok2 Nil ((:<) x0 (IntegerLit (negate x1)))

lexTok17 str@(c::cs) x1 x0 =
  case c of
    '.' => lexTok18 cs x1 x0
    _   => lexTok2 str ((:<) x0 (fromIdents x1))
lexTok17 [] x1 x0 = lexTok2 Nil ((:<) x0 (fromIdents x1))

lexTok18 str@(c::cs) x1 x0 =
  case isIdentStart c of
    True => lexTok19 cs ((:<) Lin c) x1 x0
    _    => Left (cast (Unexpected c))
lexTok18 [] x1 x0 = Left (cast EOI)

lexTok19 str@(c::cs) x2 x1 x0 =
  case isIdentTrailing c of
    True => lexTok19 cs ((:<) x2 c) x1 x0
    _    => lexTok17 str ((:<) x1 (pack ((<>>) x2 Nil))) x0
lexTok19 [] x2 x1 x0 = lexTok17 Nil ((:<) x1 (pack ((<>>) x2 Nil))) x0
