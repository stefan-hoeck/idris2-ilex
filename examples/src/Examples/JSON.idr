...
Prog o = AsyncPull Poll o [ByteErr Void, ParseError Void, Errno]
...
streamVals pths =
     flatMap pths (\p => streamParseFrom (FileSrc p) jsonArray (readBytes {size = 0x3fff} p))
  |> C.count
  |> foreach (\x => stdoutLn "\{show x} values streamed.")

covering
main : IO ()
main = runProg $ locError (InnerError Void) $ streamVals (P.tail args)
