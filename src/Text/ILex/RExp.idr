...
||| Conversion instruction for lexicographic tokens.
|||
...
||| states, so the correspond to `Bottom`.
|||
||| Parameters:
|||   `e` : Custom error type raised by the lexer
|||   `c` : Type for status codes. For simple lexers, this might just
|||         be `Unit`, but more involved lexer might allow certain
|||         kinds of tokens only in certain contexts.
|||         This describes the context to use after each successful
|||         lexing step.
|||   `a` : The type of token emitted by this lexer.
public export
data Conv : (e,c,a : Type) -> Type where
  ...
  ||| Marks a constant token that needs not be generated by
  ||| (re)inspecting the bytestring corresponding to the token
  Const  : a -> Conv e c a

  ||| Describes how to generate a token from its bytestring.
  ||| For greater flexibility, this comes with the capability
  ||| of throwing custom errors.
  Txt    : (ByteString -> Either e a) -> Conv e c a

  ||| An error token. This can be used for custom error messages.
  Err    : e -> Conv e c a

export %inline
txt : (ByteString -> a) -> Conv e c a
txt f = Txt (Right . f)
