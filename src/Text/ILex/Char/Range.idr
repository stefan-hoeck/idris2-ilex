...
public export
data RangeOf : (t : Type) -> (rel : (x,y : t) -> Type) -> Type where
  Empty : {0 rel : _} -> RangeOf t rel
  Rng   : {0 rel : _} -> (low,up : t) -> (0 p : rel low up) => RangeOf t rel

export
Show t => Show (RangeOf t rel) where
  showPrec p Empty     = "Empty"
  showPrec p (Rng x y) = showCon p "Rng" (showArg x ++ showArg y)

export
Eq t => Eq (RangeOf t rel) where
  Empty   == Empty   = True
  Rng a b == Rng x y = a == x && b == y
  _       == _       = False

export
Ord t => Ord (RangeOf t rel) where
  compare (Rng a b) (Rng x y) =
    case compare a x of
      EQ => compare b y
      c  => c
